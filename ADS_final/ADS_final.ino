...
float R1 = 900000.0;  // 900kΩ
float R2 = 43500.0;   // 44kΩ
...
void setup() {
  ...
  // Init ADS
  if (!ads1.begin(0x48)) { Serial.println("❌ ADS1115 #1 not found"); while (1); }
  if (!ads2.begin(0x49)) { Serial.println("❌ ADS1115 #2 not found"); while (1); }
  if (!ads3.begin(0x4A)) { Serial.println("❌ ADS1115 #3 not found"); while (1); }
  if (!ads4.begin(0x4B)) { Serial.println("❌ ADS1115 #4 not found"); while (1); }
...
void loop() {
  ...
  if (digitalRead(buttonPin) == LOW) {
    Serial.println("🔘 Button Pressed. Measuring for 5 seconds...");
  ...
  if (Serial.available()) {
    ...
    if (input.length() > 0) {
      ...
      for (int i = 0; i < numCells; i++) {
        ...
        if (nextComma == -1) { 
          // Last value case (no comma after it)
          cells[i].maxV = input.substring(lastComma + 1).toFloat();
          break;
        }
      ...
      saveThresholds();
      Serial.println("✅ Thresholds updated via Serial:");
      printThresholds();
    }
  }
}
